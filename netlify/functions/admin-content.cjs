  ...
  try {
    ...
    switch (httpMethod) {
      case 'POST': {
        ...
        // Validate arrays
        const arrayErrors = [];
        if (data.keywords && !Array.isArray(data.keywords)) arrayErrors.push('Keywords must be an array');
        if (data.categories && !Array.isArray(data.categories)) arrayErrors.push('Categories must be an array');
